 ...
 */
TOBJ10F0 sBackupparameterhandling = {1, 0x00};
 ...
 */
OBJCONST UCHAR OBJMEM aName0x10F0[] = "Backup parameter handling\000Checksum\000\377";
#endif // BACKUP_PARAMETER_SUPPORTED
...
#if BACKUP_PARAMETER_SUPPORTED
    /* Object 0x10F0 */
   {NULL,NULL,  0x10F0, {DEFTYPE_RECORD, 1 | (OBJCODE_REC << 8)}, asEntryDesc0x10F0, aName0x10F0, &sBackupparameterhandling, NULL, NULL, 0x0000 },
#endif // BACKUP_PARAMETER_SUPPORTED
#if !_PIC18
    /* Object 0x10F1 */
...
#endif
    {
        if (pObjEntry->NonVolatileOffset != 0)
        {
            ...
            for (i = 0; i <= maxSubindex; i++)
            {
                ...
                OBJCONST TSDOINFOENTRYDESC OBJMEM *pEntry = OBJ_GetEntryDesc(pObjEntry, i);

                if (pEntry->ObjAccess & OBJACCESS_BACKUP)
                {
                    /* entry is a backup entry */
                    UINT16 wordOffset = OBJ_GetEntryOffset(i, pObjEntry) >> 4;
